  ...
  implements IPdfGenerator<CreatePdfAcademicoDto>
{
  ...
  async pdfGenerate(dto: CreatePdfAcademicoDto): Promise<Buffer> {
    const { nombreEstudiante, rutEstudiante, carrera, resumen, semestres } =
      dto;
      ...
      content: [
        ...
        InformeBuilder.tableBuilder(
          ...
          [
            ...
            [
              'Ramos aprobados',
              `${resumen.ramosAprobados}${pct(resumen.ramosAprobados)}`,
            ],
            [
              'Ramos reprobados',
              `${resumen.ramosReprobados}${pct(resumen.ramosReprobados)}`,
            ],
            [
              'Ramos en curso',
              `${resumen.ramosCursando}${pct(resumen.ramosCursando)}`,
            ],
            [
              'Ramos eliminados',
              `${resumen.ramosEliminados}${pct(resumen.ramosEliminados)}`,
            ],
            [
              'Promedio general',
