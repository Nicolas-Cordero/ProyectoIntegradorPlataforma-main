...
export class PdfAcuerdoGenerator implements IPdfGenerator<CreatePdfAcuerdoDto> {
  ...
  async pdfGenerate(dto: CreatePdfAcuerdoDto): Promise<Buffer> {
    ...
    const seccionesTopicos: Content[] = topicos.flatMap((topico) => [
      { text: topico.nombre, style: 'header', margin: [0, 16, 0, 6] as [number, number, number, number] },
    ...
    const docDefinition = {
      content: [
        ...
        {
          stack: [
            { text: titulo,    fontSize: 18, bold: true,  margin: [0, 0, 0, 4] as [number, number, number, number] },
            { text: subtitulo, fontSize: 12, color: 'gray', margin: [0, 0, 0, 2] as [number, number, number, number] },
            { text: `Versión: ${version}`, fontSize: 9, color: 'gray', italics: true },
          ],
          margin: [0, 0, 0, 16] as [number, number, number, number],
        },
        { text: 'Resumen', style: 'header', margin: [0, 0, 0, 6] as [number, number, number, number] },
